...
def build_weighted_average(
    ...
    normalized = {column: float(weights[column]) for column in pred_columns}
    total = sum(normalized.values())
    if total <= 0.0:
        raise RuntimeError("Weighted ensemble requires positive total weight.")
    normalized = {column: value / total for column, value in normalized.items()}
    prediction = sum(validation[column] * normalized[column] for column in pred_columns)
    return pd.Series(prediction, index=validation.index), {"weights": normalized}


def build_horizon_weighted_average(
    validation: pd.DataFrame,
    pred_columns: list[str],
    near_weights: dict[str, float],
    far_weights: dict[str, float],
    split_horizon: int,
) -> tuple[pd.Series, dict[str, Any]]:
    if split_horizon < 1:
        raise RuntimeError("split_horizon must be positive.")
    near_total = sum(float(near_weights[column]) for column in pred_columns)
    far_total = sum(float(far_weights[column]) for column in pred_columns)
    if near_total <= 0.0 or far_total <= 0.0:
        raise RuntimeError("Horizon-weighted ensemble requires positive total weights.")
    normalized_near = {column: float(near_weights[column]) / near_total for column in pred_columns}
    normalized_far = {column: float(far_weights[column]) / far_total for column in pred_columns}

    prediction_values: list[float] = []
    for _, row in validation.iterrows():
        horizon = int(row["horizon_q"])
        active = normalized_near if horizon <= split_horizon else normalized_far
        prediction_values.append(float(sum(float(row[column]) * active[column] for column in pred_columns)))
    return pd.Series(prediction_values, index=validation.index), {
        "near_weights": normalized_near,
        "far_weights": normalized_far,
        "split_horizon": int(split_horizon),
    }
...
def combine_forecasts(
    ...
    if method == "simple_average":
    ...
    elif method in {"inverse_rmse", "weighted_average"}:
        weights = metadata["weights"]
        forecast = sum(future_frame[column.replace("y_pred", "forecast")] * weights[column] for column in pred_columns)
        lower_90 = sum(future_frame[column.replace("y_pred", "lower_90")] * weights[column] for column in pred_columns)
        upper_90 = sum(future_frame[column.replace("y_pred", "upper_90")] * weights[column] for column in pred_columns)
        lower_50 = sum(future_frame[column.replace("y_pred", "lower_50")] * weights[column] for column in pred_columns)
        upper_50 = sum(future_frame[column.replace("y_pred", "upper_50")] * weights[column] for column in pred_columns)
    elif method == "horizon_weighted_average":
        split_horizon = int(metadata["split_horizon"])
        forecast_values: list[float] = []
        lower_90_values: list[float] = []
        upper_90_values: list[float] = []
        lower_50_values: list[float] = []
        upper_50_values: list[float] = []
        for horizon, (_, row) in enumerate(future_frame.iterrows(), start=1):
            weights = metadata["near_weights"] if horizon <= split_horizon else metadata["far_weights"]
            forecast_values.append(float(sum(float(row[column.replace("y_pred", "forecast")]) * weights[column] for column in pred_columns)))
            lower_90_values.append(float(sum(float(row[column.replace("y_pred", "lower_90")]) * weights[column] for column in pred_columns)))
            upper_90_values.append(float(sum(float(row[column.replace("y_pred", "upper_90")]) * weights[column] for column in pred_columns)))
            lower_50_values.append(float(sum(float(row[column.replace("y_pred", "lower_50")]) * weights[column] for column in pred_columns)))
            upper_50_values.append(float(sum(float(row[column.replace("y_pred", "upper_50")]) * weights[column] for column in pred_columns)))
        forecast = pd.Series(forecast_values, index=future_frame.index)
        lower_90 = pd.Series(lower_90_values, index=future_frame.index)
        upper_90 = pd.Series(upper_90_values, index=future_frame.index)
        lower_50 = pd.Series(lower_50_values, index=future_frame.index)
        upper_50 = pd.Series(upper_50_values, index=future_frame.index)
...
def run_method(
    ...
    if method == "simple_average":
        ...
        ensemble_pred, method_metadata = build_weighted_average(validation, pred_columns, metadata["weights"])
        ensemble_param_count = len(pred_columns)
    elif method == "horizon_weighted_average":
        ensemble_pred, method_metadata = build_horizon_weighted_average(
            validation,
            pred_columns,
            metadata["near_weights"],
            metadata["far_weights"],
            metadata["split_horizon"],
        )
        ensemble_param_count = 2 * len(pred_columns) + 1
    elif method == "ridge_stacking":
    ...
    metrics_payload = {
        ...
        "model_class": "Model Ensemble",
        "champion_eligible": True,
        "description": metadata.get("description", f"{method} ensemble of {', '.join(component_names)}"),
...
def main() -> int:
    ...
    methods_to_run: list[tuple[str, dict[str, Any]]] = [(method, {}) for method in METHODS]
    lowered_experiment_id = args.experiment_id.lower()
    component_names = [component["component_name"] for component in components]
    if "refine" in lowered_experiment_id and len(components) == 2:
        first, second = pred_columns
        ...
        methods_to_run.extend(
            [
                (
                    "weighted_average",
                    {
                        "weights": {first: 0.45, second: 0.55},
                        "description": f"static 45/55 ensemble of {first_name}, {second_name}",
                    },
                ),
                (
                    "weighted_average",
                    {
                        ...
                        "description": f"static 50/50 ensemble of {first_name}, {second_name}",
                    },
                ),
                (
                    "weighted_average",
                    {
                        "weights": {first: 0.55, second: 0.45},
                        "description": f"static 55/45 ensemble of {first_name}, {second_name}",
                    },
                ),
                (
                    "weighted_average",
                    {
                        "weights": {first: 0.6, second: 0.4},
                        "description": f"static 60/40 ensemble of {first_name}, {second_name}",
                    },
                ),
                (
                    "horizon_weighted_average",
                    {
                        "near_weights": {first: 0.6, second: 0.4},
                        "far_weights": {first: 0.4, second: 0.6},
                        "split_horizon": 12,
                        "description": f"near/far 60/40-40/60 ensemble of {first_name}, {second_name}",
                    },
                ),
                (
                    "horizon_weighted_average",
                    {
                        "near_weights": {first: 0.55, second: 0.45},
                        "far_weights": {first: 0.45, second: 0.55},
                        "split_horizon": 16,
                        "description": f"near/far 55/45-45/55 ensemble of {first_name}, {second_name}",
                    },
                ),
            ]
        )

    for index, (method, metadata) in enumerate(methods_to_run, start=1):
        run_method(
    ...
    (experiment_dir / "notes.md").write_text(
        ...
        + "\nMethods:\n"
        + "\n".join(f"- {method}" for method, _ in methods_to_run)
        + "\n",
        encoding="utf-8",
    )
