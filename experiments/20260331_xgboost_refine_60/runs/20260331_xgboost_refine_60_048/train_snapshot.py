...
def generate_gradient_boosting_refinement_candidate_specs(search_frame: pd.DataFrame) -> list[dict[str, Any]]:
    candidate_sets = [
        ["consumer_confidence"],
        ["consumer_confidence", "housing_inventory"],
        ["consumer_confidence", "fed_funds"],
        [],
    ]
    available = set(search_frame.columns)
    unique_sets: list[list[str]] = []
    seen: set[tuple[str, ...]] = set()
    for exogenous in candidate_sets:
        filtered = tuple(column for column in exogenous if column in available)
        if filtered not in seen:
            seen.add(filtered)
            unique_sets.append(list(filtered))

    candidates: list[dict[str, Any]] = []
    for exogenous in unique_sets:
        for target_lags in [[1, 2, 4], [1, 2, 4, 8]]:
            for n_estimators in [300, 500, 700]:
...
def generate_xgboost_refinement_candidate_specs(search_frame: pd.DataFrame) -> list[dict[str, Any]]:
    candidate_sets = [
        ["consumer_confidence"],
        ["consumer_confidence", "housing_inventory"],
        ["consumer_confidence", "fed_funds"],
...
def generate_nbeats_refinement_candidate_specs(search_frame: pd.DataFrame) -> list[dict[str, Any]]:
    candidate_sets = [
        ["housing_inventory"],
        ["consumer_confidence"],
        ["housing_inventory", "consumer_confidence"],
        [],
    ]
    available = set(search_frame.columns)
    unique_sets: list[list[str]] = []
...
def generate_transformer_refinement_candidate_specs(search_frame: pd.DataFrame) -> list[dict[str, Any]]:
    candidate_sets = [
        ["consumer_confidence"],
        ["consumer_confidence", "housing_inventory"],
        ["consumer_confidence", "fed_funds"],
    ...
    for exogenous in unique_sets:
        for lookback in [6, 8, 10]:
            for model_dim, num_heads in [(16, 2), (24, 2), (32, 4)]:
...
def run_experiment(args: argparse.Namespace) -> int:
    ...
    if args.model_class == "ARIMAX":
        ...
        if refinement == 1:
            candidates = generate_gradient_boosting_refinement_candidate_specs(search_frame)
        else:
        ...
        if refinement == 1:
            candidates = generate_xgboost_refinement_candidate_specs(search_frame)
        else:
        ...
        if refinement == 1:
            candidates = generate_nbeats_refinement_candidate_specs(search_frame)
        else:
            candidates = generate_nbeats_candidate_specs(search_frame)
    elif args.model_class == "Transformer":
        if refinement == 1:
            candidates = generate_transformer_refinement_candidate_specs(search_frame)
        else:
