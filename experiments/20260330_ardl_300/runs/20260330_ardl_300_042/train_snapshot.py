...
def resolve_dependency_scope(model_class: str) -> str:
    if model_class == "ARIMAX-GARCH":
        return "garch"
    if model_class in {"ARIMAX", "ARDL", "VAR", "VECM", "State-Space", "Dynamic Factor", "BVAR", "Threshold VAR", "Threshold VAR / SETAR"}:
        return "econometric"
...
def walk_forward_validation_ardl(
    ...
    for origin in origins:
        ...
        predicted = pd.Series(
            np.asarray(
                fitted.forecast(steps=len(future), exog=build_exog(future, exog_columns)),
                dtype=float,
            ),
            index=future.index,
        )
