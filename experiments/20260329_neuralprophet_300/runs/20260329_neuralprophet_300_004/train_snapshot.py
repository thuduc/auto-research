...
def resolve_dependency_scope(model_class: str) -> str:
    ...
    if model_class in {"ARIMAX", "VAR", "VECM", "State-Space", "Dynamic Factor", "BVAR", "Threshold VAR", "Threshold VAR / SETAR"}:
        return "econometric"
    if model_class in {"Ridge / Lasso / Elastic Net", "Random Forest", "Gradient Boosting", "Support Vector Regression", "XGBoost", "LightGBM", "NeuralProphet"}:
        return "challenger"
    return "challenger"
...
def fit_neuralprophet(train_frame: pd.DataFrame) -> NeuralProphetFit:
    ...
    model.fit(history, freq="Q", progress="none", minimal=True)
