...
class ArimaxGarchFit:
    def __init__(self, mean_result: Any, volatility_result: Any, residual_scale: float) -> None:
...
def fit_arimax_garch(train_y: pd.Series, train_exog: pd.DataFrame | None):
    ...
    return ArimaxGarchFit(mean_result, volatility_result, residual_scale)
...
def build_final_forecast_arimax_garch(search_frame: pd.DataFrame, variable_config: dict[str, Any], output_dir: Path) -> pd.DataFrame:
    ...
    variance_forecast = fitted.volatility_result.forecast(horizon=120, reindex=False).variance
    variance_row = variance_forecast.iloc[-1].to_numpy(dtype=float) / (fitted.residual_scale ** 2)
    sigma = np.sqrt(np.maximum(variance_row, 0.0))
    z_90 = float(norm.ppf(0.95))
    z_50 = float(norm.ppf(0.75))
    ...
    final = pd.DataFrame(
        {
            ...
            "hpi_lower_90": mean_forecast - z_90 * sigma,
            "hpi_upper_90": mean_forecast + z_90 * sigma,
            "hpi_lower_50": mean_forecast - z_50 * sigma,
            "hpi_upper_50": mean_forecast + z_50 * sigma,
        }
    )
    final.to_csv(output_dir / "forecast_120q.csv", index=False)
    return final
...
def execute_search(args: argparse.Namespace, output_dir: Path, logger: TrialLogger) -> dict[str, Any]:
    ...
    if args.model_class == "ARIMAX":
        ...
        validation_frame, gof_near, gof_far, rmse_1yr, rmse_3yr, naive_rmse_1yr = walk_forward_validation_arimax_garch(search_frame, manifest)
        future_forecast = build_final_forecast_arimax_garch(search_frame, variable_config, output_dir)
        residuals = pd.Series(np.asarray(fitted.mean_result.resid, dtype=float))
