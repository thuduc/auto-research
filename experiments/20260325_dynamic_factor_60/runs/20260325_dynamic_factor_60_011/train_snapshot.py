...
def walk_forward_validation_dynamic_factor(
    ...
    for origin in origins:
        ...
        forecast = fitted.get_forecast(steps=len(future))
        predicted_mean = np.asarray(forecast.predicted_mean, dtype=float)
        predicted = pd.Series(predicted_mean[:, hpi_idx], index=future.index)
        naive = pd.Series(forecast_naive_path(train["hpi"], len(future)), index=future.index)
...
def in_sample_metrics_dynamic_factor(frame: pd.DataFrame) -> tuple[Any, float, float, float]:
    ...
    endog_columns = get_active_spec()["dynamic_factor"]["endogenous"]
    hpi_idx = endog_columns.index("hpi")
    predicted = pd.Series(np.asarray(fitted.fittedvalues, dtype=float)[:, hpi_idx], index=frame.index)
    aligned = pd.DataFrame({"y": frame["hpi"], "pred": predicted}).dropna()
    insample_rmse = rmse(aligned["y"], aligned["pred"])
    naive_rmse = compute_naive_insample_rmse(frame["hpi"])
    gof = 1.0 - (insample_rmse / naive_rmse if naive_rmse else np.inf)
...
def build_final_forecast_dynamic_factor(search_frame: pd.DataFrame, output_dir: Path) -> pd.DataFrame:
    ...
    forecast_90 = fitted.get_forecast(steps=120)
    forecast_50 = fitted.get_forecast(steps=120)
    predicted = np.asarray(forecast_90.predicted_mean, dtype=float)
    conf_90 = np.asarray(forecast_90.conf_int(alpha=0.10), dtype=float)
    conf_50 = np.asarray(forecast_50.conf_int(alpha=0.50), dtype=float)

    future_dates = pd.date_range(search_frame["date"].iloc[-1] + pd.offsets.QuarterEnd(), periods=120, freq="QE-DEC")
    lower_90_idx = 2 * hpi_idx
    upper_90_idx = lower_90_idx + 1
    final = pd.DataFrame(
        {
            "date": future_dates,
            "hpi_actual": np.nan,
            "hpi_forecast": predicted[:, hpi_idx],
            "hpi_lower_90": conf_90[:, lower_90_idx],
            "hpi_upper_90": conf_90[:, upper_90_idx],
            "hpi_lower_50": conf_50[:, lower_90_idx],
            "hpi_upper_50": conf_50[:, upper_90_idx],
        }
    )
    final.to_csv(output_dir / "forecast_120q.csv", index=False)
    return final
...
def generate_dynamic_factor_candidate_specs(search_frame: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for endogenous in unique_endogenous:
        max_factors = min(2, len(endogenous) - 1)
        for factor_order in [1, 2, 4]:
            k_factors = 1 if max_factors == 1 else min(2, max_factors)
            candidates.append(build_dynamic_factor_spec(factor_order, endogenous, k_factors=k_factors))
    return candidates
