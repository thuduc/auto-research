...
def is_refinement_experiment(experiment_id: str) -> bool:
    return "refine" in experiment_id.lower()
...
def generate_threshold_var_candidate_specs(search_frame: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    available = [column for column in preferred_variables if column in search_frame.columns]
    candidate_shapes = [
        (["hpi", "mortgage_rate"], "hpi"),
        (["hpi", "mortgage_rate"], "mortgage_rate"),
        (["hpi", "mortgage_rate", "unemployment_rate"], "mortgage_rate"),
        (["hpi", "mortgage_rate", "per_capita_income"], "hpi"),
        (["hpi", "mortgage_rate", "real_gdp"], "mortgage_rate"),
        ...
        (["hpi", "mortgage_rate", "unemployment_rate", "per_capita_income"], "mortgage_rate"),
...
def run_experiment(args: argparse.Namespace) -> int:
    ...
    experiment_dir.mkdir(parents=True, exist_ok=True)
    refinement = is_refinement_experiment(args.experiment_id)
    ...
    if args.model_class == "ARIMAX":
        ...
        candidates = (
            generate_state_space_refinement_candidate_specs(search_frame)
            if refinement
            else generate_state_space_candidate_specs(search_frame)
        )
    elif args.model_class == "Dynamic Factor":
        candidates = generate_dynamic_factor_candidate_specs(search_frame)
    elif args.model_class == "VAR":
        candidates = generate_var_refinement_candidate_specs(search_frame) if refinement else generate_var_candidate_specs(search_frame)
    elif args.model_class == "BVAR":
        candidates = generate_bvar_refinement_candidate_specs(search_frame) if refinement else generate_bvar_candidate_specs(search_frame)
    elif args.model_class in {"Threshold VAR", "Threshold VAR / SETAR"}:
        candidates = (
            generate_threshold_var_refinement_candidate_specs(search_frame)
            if refinement
            else generate_threshold_var_candidate_specs(search_frame)
        )
    elif args.model_class == "VECM":
        candidates = generate_vecm_refinement_candidate_specs(search_frame) if refinement else generate_vecm_candidate_specs(search_frame)
    else:
