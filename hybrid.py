...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--long-class", default="VECM")
    parser.add_argument("--near-class", default="State-Space")
...
def run_method(
    ...
    metrics = {
        ...
        "model_class": "Horizon Hybrid",
        "champion_eligible": True,
        "description": f"{method} hybrid of {state_component['model_class']} (near) and {vecm_component['model_class']} (far)",
...
def main() -> int:
    ...
    vecm_component = component_info(leaderboard_best(args.long_class))
    state_component = component_info(leaderboard_best(args.near_class))
    ...
    (experiment_dir / "notes.md").write_text(
        ...
        f"- {state_component['model_class']}: {state_component['experiment_id']} / {state_component['trial_id']}\n"
        "Methods:\n"
        "- hard_switch_12\n"
        "- smooth_blend_9_16\n",
        encoding="utf-8",
